...
function Home({ session }: { session: any }) {
  ...
  const fetchConfirmedMatches = async () => {
    ...
    const all = [
      ...(asRequester || []).map((b: any) => ({ ...b, role: 'requester' })),
      ...asOrganizer.map((b: any) => ({ ...b, role: 'organizer' }))
    ].sort((a, b) => (a.activities?.date || '') > (b.activities?.date || '') ? 1 : -1)
    setConfirmedMatches(all)
  }
                ...
                return (
                  <div key={m.id} onClick={() => setSelectedMatch(m)}
                    ...
                    <div className="text-center min-w-8">
                      <p className="text-lg font-medium text-green-500 leading-none">{new Date(activity?.date).getDate()}</p>
                      <p className="text-xs text-gray-400 uppercase">{new Date(activity?.date).toLocaleDateString('sv-SE', { month: 'short' })}</p>
                    </div>
                    <div className="w-px h-8 bg-gray-100" />
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-800">vs {opponent}</p>
                      <p className="text-xs text-gray-400">{activity?.location} · {activity?.time?.substring(0, 5)}</p>
                    </div>
...
function CreateActivity({ team, onCreated, onBack }: { team: any, onCreated: () => void, onBack: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-gray-50 max-w-sm mx-auto flex flex-col z-50">
      ...
      <div className="flex-1 p-4 space-y-4 overflow-y-auto pb-24">
        <div><label className="text-sm text-gray-500 mb-1 block">Typ av aktivitet *</label><select value={form.type} onChange={e => update('type', e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm outline-none focus:border-green-400 bg-white"><option value="">Välj typ</option>{(config.activity_type || []).map((c: any) => <option key={c.value} value={c.value}>{c.label}</option>)}</select></div>
  ...
  return (
    <div className="fixed inset-0 bg-gray-50 max-w-sm mx-auto flex flex-col z-50">
      ...
      <div className="bg-white border-b border-gray-100 p-3 space-y-2">
        <div className="grid-2">
          <select value={filters.type} onChange={e => updateFilter('type', e.target.value)} className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm outline-none focus:border-green-400 bg-white"><option value="">Alla typer</option>{(config.activity_type || []).map((c: any) => <option key={c.value} value={c.label}>{c.label}</option>)}</select>
