...
const registrationTypes = ['Extern länk','Email','Via Matchit','Kontakta arrangören']
...
export function CupsView({ session, onBack }: { session: any, onBack: () => void }) {
  ...
  const fetchCups = async () => {
    setLoading(true)
    let query = supabase.from('cups').select('*, teams(name)').eq('status', 'active')
  ...
  if (selectedCup) return <CupDetail cup={selectedCup} onBack={() => setSelectedCup(null)} />
      ...
      <div className="bg-green-500 p-5">
        <div className="flex justify-between items-center mb-3">
          ...
          <button onClick={() => setShowCreate(true)}
            style={{ background: 'rgba(255,255,255,0.2)', borderRadius: '10px', padding: '8px 12px', border: 'none', cursor: 'pointer', color: 'white', fontSize: '13px', fontWeight: '500' }}>
            + Lägg upp
          </button>
        </div>
      </div>
...
function CupCard({ cup, onClick }: { cup: any, onClick: () => void }) {
  ...
  const spotsLeft = cup.max_teams ? cup.max_teams - (cup.registered_teams || 0) : null
  ...
  return (
    <div onClick={onClick} className="bg-white rounded-xl border border-gray-100 overflow-hidden cursor-pointer hover:border-green-200 transition-colors">
      {cup.image_url && (
        <div style={{ height: '140px', overflow: 'hidden', background: '#f0fdf4' }}>
          <img src={cup.image_url} alt={cup.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        </div>
      )}
      <div className="p-4">
        <div className="flex justify-between items-start mb-2">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              {isFull && <span className="text-xs px-2 py-0.5 rounded-full bg-red-50 text-red-600">FULL</span>}
            </div>
            <p className="text-sm font-medium text-gray-800">{cup.name}</p>
            <p className="text-xs text-gray-400">{cup.teams?.name}</p>
          </div>
        </div>
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-xs text-gray-500">
            <span>📅</span>
            <span>{isSameDay ? dateFrom : `${dateFrom} – ${dateTo}`}</span>
          </div>
          {cup.locations?.length > 0 && (
            <div className="flex items-center gap-2 text-xs text-gray-500">
              <span>📍</span>
              <span>{cup.locations.slice(0, 2).join(', ')}{cup.locations.length > 2 ? ` +${cup.locations.length - 2}` : ''}{cup.kommun ? `, ${cup.kommun}` : ''}</span>
            </div>
          )}
          <div className="flex gap-3 text-xs text-gray-500 flex-wrap">
            {cup.formations?.length > 0 && <span>⚽ {cup.formations.join(', ')}</span>}
            {cup.levels?.length > 0 && <span>📊 {cup.levels.join(', ')}</span>}
            {cup.match_guarantee && cup.match_guarantee !== 'N/A' && <span>🎯 Min {cup.match_guarantee}</span>}
          </div>
          {cup.price > 0 && (
            <div className="text-xs text-amber-600">💰 {cup.price} kr {cup.price_type === 'Per lag' ? '/lag' : cup.price_type === 'Per spelare' ? '/spelare' : ''}</div>
          )}
          {spotsLeft !== null && !isFull && (
            <div className="text-xs text-green-600">🎟 {spotsLeft} platser kvar av {cup.max_teams}</div>
          )}
          {cup.age_groups?.length > 0 && (
            <div className="text-xs text-gray-500">👦 {cup.age_groups.join(', ')}</div>
          )}
        </div>
      </div>
    </div>
...
// ─── CupDetail ───────────────────────────────────────────────────────────────
function CupDetail({ cup, onBack }: { cup: any, onBack: () => void }) {
  ...
  const spotsLeft = cup.max_teams ? cup.max_teams - (cup.registered_teams || 0) : null
  const isFull = spotsLeft !== null && spotsLeft <= 0
  const deadline = cup.registration_deadline ? new Date(cup.registration_deadline).toLocaleDateString('sv-SE', { day: 'numeric', month: 'long' }) : null
  ...
  return (
    <div className="fixed inset-0 bg-gray-50 max-w-sm mx-auto flex flex-col z-50">
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', bottom: '16px', left: '16px', right: '16px' }}>
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            {cup.is_sanctioned && <span style={{ background: 'rgba(59,130,246,0.3)', color: 'white', fontSize: '11px', padding: '2px 8px', borderRadius: '999px' }}>✓ Sanktionerad</span>}
            {isFull && <span style={{ background: 'rgba(239,68,68,0.4)', color: 'white', fontSize: '11px', padding: '2px 8px', borderRadius: '999px' }}>FULL</span>}
          </div>
          <p className="text-white text-lg font-medium">{cup.name}</p>
          <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '12px' }}>Arrangör: {cup.teams?.name}</p>
        </div>
      </div>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-4 space-y-2">
              <div className="flex items-start gap-3 text-sm">
                ...
                <div>
                  <p className="text-gray-800 font-medium">{isSameDay ? dateFrom : `${dateFrom} – ${dateTo}`}</p>
            ...
            {cup.max_teams && (
              <div className="bg-white rounded-xl border border-gray-100 p-4">
                <div className="flex justify-between items-center mb-2">
                  <p className="text-sm font-medium text-gray-700">Anmälda lag</p>
                  <p className="text-sm font-medium text-gray-700">{cup.registered_teams || 0} / {cup.max_teams}</p>
                </div>
                <div style={{ height: '8px', background: '#f0fdf4', borderRadius: '4px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${Math.min(100, ((cup.registered_teams || 0) / cup.max_teams) * 100)}%`, background: isFull ? '#ef4444' : '#22c55e', borderRadius: '4px', transition: 'width 0.3s' }} />
                </div>
                {spotsLeft !== null && !isFull && <p className="text-xs text-green-600 mt-1">{spotsLeft} platser kvar</p>}
                {isFull && <p className="text-xs text-red-500 mt-1">Cupen är full</p>}
              </div>
            )}
            ...
            {cup.description && (
              <div style={{ background: '#f0fdf4', borderRadius: '12px', padding: '16px', border: '1px solid #bbf7d0' }}>
                <p className="text-sm font-medium text-green-800 mb-2">Om cupen</p>
                <p className="text-sm text-green-700 leading-relaxed">{cup.description}</p>
              </div>
            )}
            ...
            {(cup.has_kiosk || cup.has_changing_rooms || cup.has_parking) && (
              <div className="bg-white rounded-xl border border-gray-100 p-4">
                ...
                <div className="flex gap-3 flex-wrap">
                  {cup.has_kiosk && <span className="text-xs bg-gray-50 text-gray-600 px-3 py-1 rounded-full">🍔 Kiosk</span>}
                  {cup.has_changing_rooms && <span className="text-xs bg-gray-50 text-gray-600 px-3 py-1 rounded-full">🚿 Omklädningsrum</span>}
                  {cup.has_parking && <span className="text-xs bg-gray-50 text-gray-600 px-3 py-1 rounded-full">🅿️ Parkering</span>}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="p-4 space-y-4">
            <div className="bg-white rounded-xl border border-gray-100 p-4 space-y-3">
              ...
              {cup.price > 0 && (
                <div className="flex justify-between text-sm border-t border-gray-50 pt-2">
                  ...
                  <span className="text-gray-800 font-medium">{cup.price} kr {cup.price_type === 'Per lag' ? '/lag' : cup.price_type === 'Per spelare' ? '/spelare' : ''}</span>
                </div>
              )}
              {cup.price === 0 && (
                <div className="flex justify-between text-sm border-t border-gray-50 pt-2">
            ...
            {cup.registration_info && (
              <div style={{ background: '#fff7ed', borderRadius: '12px', padding: '16px', border: '1px solid #fed7aa' }}>
                ...
                <p className="text-sm text-amber-700">{cup.registration_info}</p>
              </div>
            )}
          </div>
        )}
      ...
      {/* Anmälningsknapp */}
      {!isFull && cup.registration_type && cup.registration_type !== 'Via Matchit' && (
...
export function CreateCup({ session, onBack }: { session: any, onBack: () => void }) {
  ...
  useEffect(() => {
    supabase.from('teams').select('*').eq('owner_id', session.user.id).single()
  ...
  const MultiSelect = ({ options, selected, onToggle, label }: { options: string[], selected: string[], onToggle: (v: string) => void, label: string }) => (
    <div>
...
{/* Progress */}
      <div style={{ display: 'flex', background: 'white', borderBottom: '0.5px solid #e5e7eb' }}>
        {['Grundinfo', 'Format & nivå', 'Anmälan'].map((s, i) => (
          <button key={i} onClick={() => setStep(i + 1)}
            style={{ flex: 1, padding: '10px 4px', fontSize: '11px', fontWeight: step === i + 1 ? '500' : '400',
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-24">
          ...
          <>
            ...
            <div>
              ...
              <div onClick={() => fileInputRef.current?.click()}
                style={{ height: imagePreview ? 'auto' : '120px', border: '2px dashed #e5e7eb', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', overflow: 'hidden', background: '#f9fafb' }}>
                {imagePreview ? (
                  ...
                  <div style={{ textAlign: 'center', padding: '16px' }}>
                    ...
                    <p className="text-xs text-gray-300">JPG, PNG – rekommenderat 1200x600px</p>
                  </div>
                )}
              </div>
              ...
              {imagePreview && (
                <button onClick={() => { setImageFile(null); setImagePreview('') }}
                  className="text-xs text-red-400 mt-1">Ta bort bild</button>
              )}
            </div>

            <div>
              <label className="text-sm text-gray-500 mb-1 block">Typ *</label>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button onClick={addLocation}
                  className="bg-green-500 text-white px-4 py-3 rounded-xl text-sm font-medium">+</button>
              </div>
                  ...
                  {form.locations.map(loc => (
                    <span key={loc} style={{ background: '#f0fdf4', color: '#166534', fontSize: '12px', padding: '4px 10px', borderRadius: '999px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      ...
                      <button onClick={() => update('locations', form.locations.filter(l => l !== loc))}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#16a34a', fontSize: '14px', lineHeight: 1 }}>×</button>
                    </span>
                  ))}
                </div>
              )}
            ...
            <div>
              <label className="text-sm text-gray-500 mb-1 block">Beskrivning / informationsruta</label>
              <textarea value={form.description} onChange={e => update('description', e.target.value)}
                placeholder="Berätta om cupen – format, regler, vad som ingår, varför lag ska delta..."
        ...
        {step === 3 && (
          <>
            <div>
              ...
              <div className="space-y-2">
                {registrationTypes.map(rt => (
                  <label key={rt} className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-gray-200 hover:border-green-300 transition-colors"
                    style={{ borderColor: form.registration_type === rt ? '#22c55e' : '' }}>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        ...
                        {rt === 'Email' && 'Lag öppnar sin email-app och kontaktar dig'}
                        {rt === 'Via Matchit' && 'Lag anmäler sig direkt i appen'}
                        {rt === 'Kontakta arrangören' && 'Visa kontaktinfo och instruktioner'}
                      </p>
                    </div>
                  </label>
                ))}
            ...
            <div>
              <label className="text-sm text-gray-500 mb-1 block">Ytterligare anmälningsinformation</label>
              <textarea value={form.registration_info} onChange={e => update('registration_info', e.target.value)}
                placeholder="t.ex. Ange lagnamn, kontaktperson och telefonnummer i anmälan..."
            ...
            <div style={{ background: '#f0fdf4', borderRadius: '12px', padding: '12px' }}>
              <p className="text-xs font-medium text-green-800 mb-2">Anmälningsknapp – förhandsgranskning</p>
              <button style={{ width: '100%', background: '#22c55e', color: 'white', padding: '12px', borderRadius: '12px', border: 'none', fontSize: '14px', fontWeight: '500', cursor: 'default' }}>
                {form.registration_type === 'Extern länk' && '🔗 Anmäl via extern sida'}
                {form.registration_type === 'Email' && '✉️ Anmäl via email'}
                {form.registration_type === 'Via Matchit' && '⚽ Anmäl via Matchit'}
